<?php
  ...
  function registerUser($dbh, $firstname, $lastname, $gender, $username, $password, $email, $age) {
    $query = "INSERT INTO users VALUES ('', '$firstname', '$lastname', '$gender', '$age', '$username', '$password', '$email', '1', 'now()')";
    $result = mysqli_query($dbh, $query) or die(mysqli_connect_errno($dbh));
    if ($result) {
  ...
  function addThread($dbh, $name, $description, $cat_id) {
    //Y-m-d H:i:s
    //2016-08-08 17:05:10
    $date = Date('Y-m-d H:i:s');
    $query = "INSERT INTO threads VALUES ('', '$name', '$date', '$description', '$cat_id', '4')";
    $result = mysqli_query($dbh, $query) or die(mysqli_connect_errno($dbh));
    if ($result) {
      return true;
    } else return false;
